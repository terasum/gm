...
type SM2CurveParams struct{
	...
	A       *big.Int // the A constant of the curve equation
...
func GenerateKey(curve SM2Curve, rand io.Reader) (priv []byte, x, y *big.Int, err error) {
	N := curve.Params().N
	bitSize := N.BitLen()
	byteLen := (bitSize + 7) >> 3
...
func Marshal(curve SM2Curve, x, y *big.Int) []byte {
...
func Unmarshal(curve SM2Curve, data []byte) (x, y *big.Int) {
	byteLen := (curve.Params().BitSize + 7) >> 3
